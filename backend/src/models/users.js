/**
 * Model: users
 * Database model wrapper for users table
...
class UsersModel {
  ...
  async create(data) {
    ...
    logger.info('users created', { id: res.rows[0].id });
    return res.rows[0];
  }
  ...
  async search(searchTerm, limit = 20) {
    const query = `SELECT * FROM users WHERE to_tsvector('english', coalesce(name,'') || ' ' || coalesce(description,'')) @@ plainto_tsquery('english', $1) LIMIT $2`;
    try {
      const res = await db.query(query, [searchTerm, limit]);
      return res.rows;
    } catch {
      const res = await db.query(`SELECT * FROM users WHERE name ILIKE $1 LIMIT $2`, [`%${searchTerm}%`, limit]);
      return res.rows;
    }
  }
}
